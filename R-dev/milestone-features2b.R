...
x <-  foreach( measure = ifeatures( measures[1:2] ), .combine = comb  ) %:%   # events
  foreach( direction = directions[1:2], .combine = comb  ) %:%            # characer: before, after, within
  foreach( span = spans, .combine = comb # , .multicombine=TRUE
        , .packages=c('whisker','ML.tools','dp.misc','na.actions'), .export=cqq(feature_prefix, time_grain) ) %do%                   # durations: 
    {

...
#' @param event character; name of event_dt column
#' @param direction; before, after, within 
#' @param span length of time for the window 
#' 
#' Window := direction + span 
#' 
#' before: Milestone - event <= window  (LEADING INDICATOR)
#' after : Milestone - event >= window  (LAGGING INDICATOR)
#' within (of): abs(Milestone - event ) <= windows     
#' 
#' @return 
...
window_expr <- function( milestone, event, direction="within", span ) {
  
  span <- as.numeric(span)
  
  template <- switch( direction  
      , before = '{{milestone}} - {{event}} <= {{span}}'
      , after  =  '{{event}} - {{milestone}} <= {{span}}'
      , within = 'abs( {{milestone}} - {{event}} ) <= {{span}}' 
  )
  
  tdata <- list( 
      milestone = milestone
    , event = event    # name of column on the event data set
    , span = span
  )
